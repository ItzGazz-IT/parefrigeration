...
async function main() {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      const [insertResult] = await connection.query(
        `INSERT INTO scan_out_events (
          ...
          scanned_at,
          created_at
        ) VALUES (?, ?, ?, ?, 'TFF_DEALER', ?, ?, ?, 0, 'tff_dealer_scans', 'SOLD', ?, ?)`,
        [
          unit.id || null,
      ...
      if (ioNumber) {
        const [existingArchive] = await connection.query(
          `SELECT id FROM archive_records WHERE serial_number = ? AND scan_type = 'TFF_DEALER' LIMIT 1`,
          [serialNumber]
        );
        ...
        if (!existingArchive.length) {
          await connection.query(
            `INSERT INTO archive_records (
              ...
              archived_at,
              created_at
            ) VALUES (?, 'TFF_DEALER', ?, ?, ?, NOW(), NOW())`,
            [serialNumber, ioNumber, insertResult.insertId, row.client_name || null]
          );
          archived += 1;
        }
      }
    ...
    console.log(`Backfill complete. created=${created} archived=${archived}`);
  } finally {
...
main().catch((error) => {
  console.error('backfill_tff_dealer_events failed:', error.message);
  process.exit(1);
});
